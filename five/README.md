# Five — Score.dev Workload Example

This project demonstrates how to deploy the `five` workload using [Score.dev](https://score.dev), via:
...
```bash
score-compose resources get-outputs dns.default#five.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#five.dns --format '{{ .host }}'
```
